...
LL44_CACHE_FILE = "data/raw/ll44_funding_data.csv"
LL44_CACHE_MAX_AGE_HOURS = 24  # Consider cache valid for 24 hours
...
def query_and_add_financing(hpd_csv_path, output_path=None, use_cache=True):
    """
    ...
    Args:
        hpd_csv_path: Path to HPD data CSV
        output_path: Path to save updated CSV
        use_cache: Whether to use local LL44 cache (default True)
    """
    ...
    # Query LL44 funding database (with caching)
    ll44_funding = query_ll44_funding(project_ids, use_cache=use_cache)
